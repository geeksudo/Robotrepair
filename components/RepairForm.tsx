...
import { Customer, ProductModel, RepairRecord, PartAction, RepairActionType, Part } from '../types';
...
export const RepairForm: React.FC<RepairFormProps> = ({ availableParts, onCancel, onSave }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <button onClick={onCancel} className="mb-6 flex items-center text-gray-500 hover:text-gray-900 transition-colors">
      ...
      <div className="bg-white shadow-xl rounded-2xl overflow-hidden">
        <div className="bg-green-600 px-8 py-6">
          <h2 className="text-2xl font-bold text-white flex items-center">
            <IconRobot className="w-8 h-8 mr-3 text-green-100" />
            New Repair Entry
          </h2>
          <p className="text-green-100 mt-1">Fill in the details below. AI will generate the customer report automatically.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-700">Area / Version</label>
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-700">Product Name (Identifier)</label>
          ...
          <div className="border-t border-gray-200 pt-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
                <h3 className="text-lg font-medium text-gray-900 mb-2 sm:mb-0">Replaced or Repaired Parts</h3>
            ...
            <div className="grid grid-cols-1 gap-6">
              {Object.entries(partsByCategory)
                ...
                <div key={category} className="bg-gray-50 rounded-lg p-4 transition-all duration-300">
                  <h4 className="text-sm font-semibold text-gray-700 uppercase tracking-wider mb-3">{category}</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {parts.map(part => {
                      ...
                      return (
                        <div key={part.id} className={`flex flex-col p-3 rounded border transition-all ${isSelected ? 'bg-white border-green-500 ring-1 ring-green-500 shadow-sm' : 'bg-white border-gray-200 hover:border-green-300'}`}>
                          <label className="flex items-start space-x-3 cursor-pointer mb-2">
                            ...
                            />
                            <span className={`text-sm font-medium ${isSelected ? 'text-green-800' : 'text-gray-900'}`}>{part.name}</span>
                          </label>
          ...
          <div className="border-t border-gray-200 pt-6">
             <label className="block text-lg font-medium text-gray-900 mb-2">Technician Notes (Internal)</label>
             <textarea 
                rows={4} 
                value={technicianNotes} 
                onChange={e => setTechnicianNotes(e.target.value)}
                placeholder="Describe the diagnosis, water damage level, specific repairs done..."
          ...
          <div className="pt-4 flex justify-end items-center space-x-4">
            <button type="button" onClick={onCancel} className="px-6 py-3 border border-gray-300 shadow-sm text-base font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
              Cancel
            </button>
            <button 
                type="submit" 
                disabled={isGenerating}
                className={`px-8 py-3 border border-transparent shadow-sm text-base font-medium rounded-md text-white ${isGenerating ? 'bg-green-400' : 'bg-green-600 hover:bg-green-700'} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 flex items-center`}
            >
              {isGenerating ? (
                <>
